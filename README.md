...
### Family reading prototype · `leo-story-practices-bar`

Reading opens a full-viewport paper stage, informed by the [Wonder Together Practices family reader](https://wonder-together.app/practices). The grown-up reads aloud; the child taps **Next**. **Back** revisits a beat, switching subjects preserves the current page and writing draft, and **Read again** starts a fresh reading. Nothing is stored across reloads. No new route, public WT navigation, accounts, CMS, or runtime AI is involved. This is Wonder Daily dogfood only; branch review precedes any deployment.

The ten beats are: **meet the inchworm → front grips → rear loops → front stretches → the measuring-like name → tiny legs / caterpillar → what holds on? → why that name? → help Leo rethink sliding → closure**. _Grip_, _loop_, and _stretch_ each have one optional meaning beside their story moment. Each discussion question gets its own screen with an optional story excerpt. The last screen offers a pause, writing, print, or a reread; it adds no lesson content. The six story passages are shared with print; `reading.beats` adds only digital titles, poses, and word references.

Four craft choices are locked for this prototype:

- **Paper and ink:** `#f7f1e6` with deep warm ink; no card stack or progress bar. Subject tabs and print stay quiet around the stage.
- **Read-aloud type:** local Newsreader at 22px on phone / 24px on tablet, 1.55 leading, and a restrained Inter interface. Generous space and a narrow passage measure give each beat room.
- **Teaching motion:** a 500ms eased opacity / perspective page turn reveals one passage. The fixed letterbox shows the front staying anchored as the rear loops, then the rear staying anchored as the front reaches. Math keeps its independent Grip → Loop → Stretch interaction. No autoplay, looping, or advance timers. Reduced motion settles immediately, including when the preference changes mid-turn; leaving Reading or rapid taps cancels the previous transition.
- **A still paper twin:** Print story opens the separate B&W-safe, one-page Letter PDF. It includes every passage, word meaning, and both response spaces. Browser Print also reveals all reader beats. With JavaScript disabled, all passages remain readable in order. Enlarged text can scroll without clipping the current beat.

Reader checks run inside `npm run test:browser`: all ten beats at the three target viewports in Chromium and WebKit; Next / Back / restart; keyboard focus; in-context words; question evidence; subject return; enlarged text; no-JS and browser-print fallbacks; anchored motion, interruption, rapid taps, and live reduced-motion changes. Tablet accessibility checks cover every beat. Review captures include `docs/stills/inchworms-ipad-reading.png`; additional beat captures are generated under `output/playwright/inchworms-reader-*.png`.

Review stills are under `docs/stills/inchworms-*`: digital Math, Reading, Writing, the loop pose, and all four grayscale print pages. The five PDFs are rebuilt in `output/pdf/` and copied into `dist/pdf/`: `math.pdf`, `reading.pdf`, `writing.pdf`, `kid-worksheets.pdf`, and the separate `parent-answer-key.pdf`. Print on US Letter at **100% / actual size**, one-sided. The page-boundary guard and PDF tests verify one page per subject, three kid pages, and a separate one-page key.

![Inchworms digital math](docs/stills/inchworms-ipad-math.png)
