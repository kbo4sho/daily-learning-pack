...
import {
  ...
  scenicPlate,
  growthStrip,
} from "./beansprout-render.mjs";
export { esc } from "./html.mjs";
const familyEdition = (p) => ["inchworms", "beansprout"].includes(p.kind);
...
const head = (title, print = false, kind = "") =>
  `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#193c39"><meta name="description" content="One topic. Three small discoveries. A daily math, reading, and writing pack."><title>${esc(title)} · Daily Learning Pack</title><link rel="stylesheet" href="${print ? "../" : "./"}styles.css">${["inchworms", "beansprout"].includes(kind) ? `<link rel="stylesheet" href="${print ? "../" : "./"}inchworms.css">` : ""}${kind === "beansprout" ? `<link rel="stylesheet" href="${print ? "../" : "./"}beansprout.css"><meta name="robots" content="noindex, nofollow">` : ""}<link rel="icon" href="data:,"></head>`;
function paperHeader(p, subject, num) {
  return `<header class="paper-head"><div class="eyebrow">${familyEdition(p) ? "WONDER DAILY" : "DAILY LEARNING PACK"} <span>${num} / 03 · ${subject.toUpperCase()} · 15 MIN</span></div><h1>${esc(p[subject].title)}</h1><p class="paper-topic">${esc(familyEdition(p) ? p.title : p.topic)} · Grade ${esc(p.gradeLevel)}</p><div class="name-line">Name __________________________ <span>Date _______________</span></div></header>`;
}
export function worksheet(p, subject) {
  const part = p[subject];
  let body;
  if (p.kind === "beansprout" && ["math", "reading"].includes(subject)) {
    body = beanPrintBody(p, subject);
  } else if (subject === "math" && p.math.workspaces) {
    ...
    const story = storyPassages(part);
    body = `<div class="print-words">${part.words.map((w) => `<span><b>${esc(w.word)}</b> · ${esc(w.meaning)}</span>`).join("")}</div><div class="paper-story">${paras(story)}</div>${part.questions.map((q, i) => step(i + 1, q, lines(1))).join("")}`;
  ...
  return `<article class="sheet ${p.math.workspaces ? `engine-sheet engine-${subject}` : ""} ${familyEdition(p) ? `inchworm-sheet inchworm-${subject}` : ""} ${p.kind === "beansprout" ? `bean-sheet bean-${subject}` : ""}">${paperHeader(p, subject, { math: "01", reading: "02", writing: "03" }[subject])}<main>${body}</main><footer class="paper-parent"><b>GROWN-UP NOTE</b> ${esc(part.parentNote)}</footer></article>`;
}
export function answerSheet(p) {
  return `<article class="sheet answer-sheet"><header class="paper-head"><div class="eyebrow">${familyEdition(p) ? "WONDER DAILY" : "DAILY LEARNING PACK"} <span>GROWN-UPS ONLY</span></div><h1>A little guidance</h1><p class="paper-topic">${esc(familyEdition(p) ? p.title : p.topic)} · Grade ${esc(p.gradeLevel)} · Parent answer key</p></header><p class="paper-intro">Three gentle sessions: 15 minutes each. Understanding matters more than finishing every line.</p><dl>${p.answers.map((a) => `<dt>${esc(a.label)}</dt><dd>${esc(a.text)}</dd>`).join("")}</dl><aside class="paper-tip">${esc(p.parentGuidance)}</aside><footer class="paper-parent">Materials: pencil, crayons if you like, and scrap paper. Print kid pages separately from this key.</footer></article>`;
}
...
function storyPassages(reading) {
  if (reading.beats?.every((beat) => beat.passage))
    return reading.beats.map((beat) => beat.passage);
  return reading.paragraphs;
}
// The print twin uses the same story passages, with no reader state or controls.
function inchwormReader(p) {
  const passages = storyPassages(p.reading);
  const cover = `<article class="reader-beat" data-reader-beat="cover" data-reader-pose="0" aria-labelledby="cover-heading"><div class="reader-copy"><p class="eyebrow">WONDER DAILY · A STORY TO OPEN</p><h3 id="cover-heading" tabindex="-1">The little leaf walker</h3><figure class="reader-letterbox reader-cover-book">${inchwormDiagram("reader-cover-worm", 0)}<figcaption>A close-up drawing · not life size</figcaption></figure><p class="reader-passage">Find a cozy spot. Take your time. Grown-up reads; little wonderer opens the story.</p><p class="reader-aside">One little page at a time.</p></div></article>`;
  const story = p.reading.beats.map((beat, i) => {
    const word = p.reading.words.find((w) => w.word === beat.word);
    return `<article class="reader-beat" data-reader-beat="story-${i}" data-reader-pose="${beat.pose}" aria-labelledby="beat-${i}-heading"><div class="reader-copy"><p class="eyebrow">${i === 0 ? "A STORY TO READ TOGETHER" : "THE LITTLE LEAF WALKER"}</p><h3 id="beat-${i}-heading" tabindex="-1">${esc(beat.title)}</h3><figure class="reader-letterbox">${inchwormDiagram(`reader-worm-${i}`, beat.pose)}<figcaption>${i === 0 ? "A close-up drawing · not life size" : i < 4 ? "The filled dot shows which end holds on." : "Look closely. Take your time."}</figcaption></figure><p class="reader-passage">${esc(beat.passage)}</p>${word ? `<details class="word reader-word"><summary>${esc(word.word)}<span> · a word to try</span></summary><p>${esc(word.meaning)}</p></details>` : i === 0 ? `<p class="reader-aside">Grown-up reads. Little wonderer taps Next.</p>` : ""}</div></article>`;
  });
  const discussion = p.reading.questions.map(
    (q, i) =>
      `<article class="reader-beat" data-reader-beat="question-${i}" aria-labelledby="question-${i}-heading"><div class="reader-copy"><p class="eyebrow">PAUSE & TALK TOGETHER</p><h3 id="question-${i}-heading" tabindex="-1">${i === 0 ? "What did you notice?" : "What’s in a name?"}</h3><p class="reader-passage">${esc(q.replace("Underline", "Find"))}</p><details class="word reader-evidence"><summary>Look back at the story</summary><p>${esc(i === 0 ? `${passages[1]} ${passages[2]}` : passages[4])}</p></details><p class="reader-aside">Say it, point it out, or talk it through together.</p></div></article>`,
  );
