...
export function worksheet(p, subject) {
  ...
  } else if (subject === "writing" && p.kind === "engines") {
  ...
  return `<article class="sheet ${p.kind === "engines" ? `engine-sheet engine-${subject}` : ""}">${paperHeader(p, subject, { math: "01", reading: "02", writing: "03" }[subject])}<main>${body}</main><footer class="paper-parent"><b>GROWN-UP NOTE</b> ${esc(part.parentNote)}</footer></article>`;
}
...
function engineLab(p) {
  ...
  <ol class="motion-steps">${p.motion.steps.map((s, i) => `<li><button data-motion-step="${i}" aria-pressed="${i === 0}" aria-controls="motion-caption"><span>${i + 1}</span>${esc(s.label)}</button><p class="motion-fallback">${esc(s.text)}</p></li>`).join("")}</ol>
...
function engineMath(p) {
  return `<div class="engine-math-grid">${engineCounter()}<div class="math-missions"><div class="mission-heading"><div><span class="eyebrow">THINK & TRY · ABOUT 9 MIN</span><h3>Help Jo reach 60.</h3></div><a class="outline-link" href="./pdf/math.pdf">Math page ↗</a></div><p>Use paper. Draw, write, or tell someone.</p>${p.math.tasks
    .slice(0, 2)
    .map(
      (t, i) =>
...
function engineReadingCheck(p) {
  return `<div class="engine-reading-check"><span class="eyebrow">HELP LEO UNDERSTAND</span><h3>${esc(p.reading.check.prompt)}</h3><div class="quiz-options">${p.reading.check.choices.map((c, i) => `<button data-engine-answer="${i}">${esc(c)}</button>`).join("")}</div><p id="engine-reading-feedback" aria-live="polite">Follow the parts in the story. You can try again.</p></div>`;
}
function lab(p) {
