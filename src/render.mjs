...
import { curiositySite, curiosityWorksheet } from "./curiosity-render.mjs";
...
const head = (title, print = false, kind = "") =>
  `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#193c39"><meta name="description" content="One topic. Three small discoveries. A daily math, reading, and writing pack."><title>${esc(title)} · Daily Learning Pack</title><link rel="stylesheet" href="${print ? "../" : "./"}styles.css">${kind === "inchworms" ? `<link rel="stylesheet" href="${print ? "../" : "./"}inchworms.css">` : ""}${isCuriosity({ kind }) ? `<link rel="stylesheet" href="${print ? "../" : "./"}curiosity.css">` : ""}<link rel="icon" href="data:,"></head>`;
function paperHeader(p, subject, num) {
  return `<header class="paper-head"><div class="eyebrow">${["inchworms"].includes(p.kind) || isCuriosity(p) ? "WONDER DAILY" : "DAILY LEARNING PACK"} <span>${num} / 03 · ${subject.toUpperCase()} · 15 MIN</span></div><h1>${esc(p[subject].title)}</h1><p class="paper-topic">${esc(["inchworms"].includes(p.kind) || isCuriosity(p) ? p.title : p.topic)} · Grade ${esc(p.gradeLevel)}</p><div class="name-line">Name __________________________ <span>Date _______________</span></div></header>`;
}
export function worksheet(p, subject) {
  if (isCuriosity(p)) return curiosityWorksheet(p, subject);
...
export function site(p, options) {
  if (isCuriosity(p)) return curiositySite(p, options);
