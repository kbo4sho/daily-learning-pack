...
export function site(p) {
  ...
  <section id="reading" class="lesson-panel" aria-labelledby="reading-heading"><div class="reading-layout"><div><div class="eyebrow">02 / READ & WONDER</div><h2 id="reading-heading" tabindex="-1">${esc(p.reading.title)}</h2><p class="section-intro">Read together. Read it again. Let the words paint a picture.</p><div class="story">${paras(p.reading.paragraphs)}</div></div><aside class="word-garden"><div class="eyebrow">LITTLE WORDS, BIG IDEAS</div><h3>Words to grow</h3><p>Tap a word to find its meaning.</p>${vocabulary(p)}${p.kind === "inchworms" ? `<div class="reading-specimen">${inchwormDiagram("reading-loop", 1)}<p>A loop lifts the middle.</p></div>` : ""}</aside></div><div class="reading-questions"><span class="eyebrow">TALK ABOUT IT · ABOUT 5 MIN</span>${p.reading.questions.map((q, i) => `<p><b>${i + 1}.</b> ${esc(q)}</p>`).join("")}<a class="outline-link" href="./pdf/reading.pdf">Reading worksheet ↗</a></div>${p.reading.check ? readingCheck(p) : ""}${finish("reading", "On to your ideas", "writing")}</section>
