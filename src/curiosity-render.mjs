...
export function curiositySite(
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex, nofollow"><meta name="theme-color" content="#f8f5ee"><title>${esc(p.title)} · Wonder Daily · Dogfood</title><link rel="icon" href="data:,"><link rel="stylesheet" href="./curiosity.css"></head><body data-kind="${esc(p.kind)}"><a class="skip" href="#daily-content">Skip to the wonder</a><div class="daily-shell"><header class="daily-header"><a class="brand" href="#curiosity" data-go="curiosity">Wonder Together<span>WONDER DAILY</span></a><a class="archive-door" href="${esc(archiveHref)}">All mornings</a><span class="dogfood-note">A family moment, in the making.<small>Private dogfood · Grade 2</small></span>${printToday}</header><nav class="daily-nav" aria-label="Explore this day">${[
    ...
    .join(
      "",
    )}</nav><main id="daily-content" tabindex="-1">${curiosity(p)}${reader(p, print)}${math(p, print)}${writing(p, print)}</main><footer class="daily-footer"><p>Small discoveries. Time together.</p><details><summary>For grown-ups & print downloads</summary><p>${esc(p.parentGuidance)}</p><p>${esc(p.reading.parentNote)}</p><p>US Letter · one-sided · 100% / actual size · black-and-white safe worksheets</p>${printFooter}<p class="small-note">Illustration explanations: ${p.sources.map((s) => `<a href="${esc(s.url)}">${esc(s.label)}</a>`).join(" · ")}</p></details><p class="small-note">Internal quality mock. Not a public launch.</p></footer></div><script id="curiosity-content" type="application/json">${JSON.stringify(p).replace(/</g, "\\u003c")}</script><script type="module" src="./curiosity.js"></script><script type="module" src="./reader.js"></script></body></html>`;
}
...
export function lineDrawing(id) {
  const potTop = "M35 28H205M45 28L57 151H183L195 28";
  const potMid = "M35 100H205M45 100L57 151H183L195 100";
  ...
  const reach = `<path d="M70 120Q90 70 130 55Q160 44 178 70M130 55Q128 90 120 130"/>`;
  const change = `<path d="M78 118L110 70M130 70L162 118M110 70H130"/><path d="M96 100H154"/>`;
  const returnLook = `<path d="M175 50Q70 40 70 100Q70 140 130 140"/><path d="M86 38L70 50 86 62"/>`;
  const keep = `<path d="M92 70H148V118H92Z"/><path d="M120 88V102"/><circle cx="120" cy="80" r="2"/>`;
  const generic = `<path d="M90 70H150M120 70V130"/>`;
  const noticeMarks = `<path d="M200 100V15M194 28H206M194 72H206M193 100H207"/>`;
  const drawings = {
    wake: {
      aria: "Stored food inside a bean, with a first root",
      label: "stored food + first root",
      pot: potTop,
      extra: seed,
    },
    root: {
      aria: "Shoot up, root down",
      label: "shoot up / root down",
      pot: potMid,
      extra: sprout,
    },
    reach: {
      aria: "A small path reaching toward the light",
      label: "a quiet reach",
      pot: potMid,
      extra: reach,
    },
    leaf: {
      aria: "Leaves above, roots below",
      label: "light + air + water",
      pot: potMid,
      extra: leaves,
    },
    change: {
      aria: "A new edge after a second look",
      label: "something changes",
      pot: potMid,
      extra: change,
    },
    notice: {
      aria: "Earlier and later height from the same soil line",
      label: "same soil baseline",
      pot: potMid,
      extra: `${leaves}${noticeMarks}`,
    },
    return: {
      aria: "Looking back at the first picture",
      label: "look once more",
      pot: potMid,
      extra: returnLook,
    },
    keep: {
      aria: "A question left open",
  ...
  const drawing = drawings[id] || {
    aria: "A quiet scene to look at together",
    label: "a quiet look",
    pot: potMid,
    extra: generic,
  };
  return `<svg viewBox="0 0 240 160" role="img" aria-label="${esc(drawing.aria)}"><g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${drawing.pot}"/>${drawing.extra}</g><text x="8" y="14" font-size="10" font-family="sans-serif">${esc(drawing.label)}</text></svg>`;
}
const lines = (n) =>
